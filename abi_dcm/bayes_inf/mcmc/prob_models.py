...
def DCM_bilinear(A, C, Ahat0, Ahat0_triu_idx, Ahat0_tril_idx, A_fract, Chat0, Chat0_nZero_idx, \
                 stim_pars_cond, i_cond, xs_exp, GD=False):
    ...
    if stim_shape=='Gamma':
        ...
        stim = stim_signal(shape=stim_shape, ntime=ntime, ninputs=ninputs, stim_onset=onset_ind, alpha=alpha, beta=beta)
    else:
        stim_tau_cond = stim_pars_cond
        stim_tau_hat = numpyro.sample(f'stim_tau_{i_cond}', dist.LogNormal(jnp.log(stim_pars_cond),1/16))
        stim = stim_signal(shape=stim_shape, ntime=ntime, ninputs=ninputs, stim_onset=onset_ind, stim_tau=stim_tau_hat)
...
def fit_model_conf(GD=False, i_cond=0):
