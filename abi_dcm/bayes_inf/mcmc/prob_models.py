...
def DCM_bilinear(A, Ahat0, Ahat0_triu_idx, Ahat0_tril_idx, A_fract, B, Chat0, Chat0_nZero_idx, \
                 s_label, stim_pars, tau, TRLs, dt, x0, eps, Lambda_max, cov_matrix, \
                 GD=False, GC_std=True, stim_sh=1, ninputs=1, ntime=1, onset_ind=0, xs_exp=None):
    ...
    ## A_hat
    A_hat = numpyro.deterministic(f'A_hat{s_label}', A_hat)
    ...
    Chat0_hat = numpyro.sample('C', dist.LogNormal(jnp.log(Chat0_vec),1/16))
    C_hat = C_hat.at[Chat0_nZero_idx].set(Chat0_hat.squeeze())
    C_hat = numpyro.deterministic(f'C_hat{s_label}',C_hat)
    ...
    if stim_sh: # Gamma shape
        ...
        theta_sigma = numpyro.sample(f'theta_sigma{s_label}', dist.LogNormal(jnp.log(alpha_cond/beta_cond**2),1/16))
        while True: # alpha must be larger than 1
            theta_mu = numpyro.sample(f'theta_mu{s_label}', dist.LogNormal(jnp.log(alpha_cond/beta_cond),1/16))
            if (theta_mu > jnp.sqrt(theta_sigma)).all:
                break
                
        alpha = numpyro.deterministic(f'alpha{s_label}', theta_mu**2/theta_sigma)
        beta = numpyro.deterministic(f'beta{s_label}', theta_mu/theta_sigma)
        ...
        stim_tau_hat = numpyro.sample(f'stim_tau{s_label}', dist.LogNormal(jnp.log(stim_pars),1/16))
    ...
    stim_hat = numpyro.deterministic(f'stim{s_label}', stim[...,ts])
    us_hat = jnp.matrix_transpose(stim_hat)
    
    ### Prior for tau ####
    tau_hat = numpyro.sample(f'tau{s_label}', dist.LogNormal(jnp.log(tau),1/16)) # in seconds
